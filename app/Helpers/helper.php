...
use Stripe\Token;
 ...
 function getImagePath($user)
 {
       $upload = Upload::where('id',$user->profile_image_id)->first();
        ...
        if(isset($image))
        {
            $profileImage =  isset($image->file_name) ? asset($image->file_name) : null;
        }
        else
        {
            $profileImage =  null;
...
function getData($user_riderequest_detail)
{
    ...
    $riderOrDriverDetail = [
        ...
        'ride_image' => (isset($ride_image)) ? asset($ride_image->file_name) : '',
...
{
    $url = 'https://maps.googleapis.com/maps/api/distancematrix/json?destinations=' . $end_latitude . '%2C' . $end_longitude . '%7C&origins=' . $start_latitude . '%2C' . $start_longitude . '&key=AIzaSyDs-RA9UNefvlGG5868vLUFR6A5frTfnAs';
