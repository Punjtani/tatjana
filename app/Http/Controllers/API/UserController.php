...
class UserController extends Controller
{
    ...
    function purchaseToken(Request $request)
    {
        ...
        return response()->json([
            'status' => true,
            'message' => 'Token Purchased Successully'
        ]);
    }
    ...
    public function profileImage(Request $request)
    {
        ...
        if(isset($upload))
        {
            ...
            ->update(['profile_image_id' => $request->id]);
        }
        $image = str_replace("public/", "", $upload->file_name);
        return response()->json([
            'status' => true,
            'data'  => ['link' => asset($image)],
            'message' => 'Data Retrieved Successfully',        
        ]);
    }
    public function getUserDetail()
    {
        $user = User::with(['riderDetail', 'cards', 'driverDetail'])
    ...
    public function riderUpdate(Request $request)
    {
        ...
        $v = DB::transaction(function () use ($request, $input) {
            ...
            $riderDetail =  RiderDetail::where('rider_id', $request->id)->update([
                'rider_id' => $request->id,
                'card_id' => 0,
                'driver_preference' => $request->driver_prefrences,
                'music_choice' => $request->music,
                'fun_fact' => $request->fun_fact,
                'next_place' => $request->next_place,
                'movie'  => $request->movie,
                'fear' => $request->fear,
                'live_without' => $request->live_without,
                'obsessed_with' => $request->obsessed_with,
        ...
        $user = User::where('id', $v->id)->with('riderDetail')->first();
    ...
    public function driverUpdate(Request $request)
    {
        ...
        $user->userDetail()->update([
            'work_address'  =>  $request->address1,
            'home_address' =>  $request->address2,
    ...
    public function register(Request $request)
    {
        ...
        $response = [
            'status' => true,
            'message' => $message,
            'data' => User::where('id', $v->id)->with('riderDetail')->first()
        ];
        return json_custom_response($response);
        // });
    }
    ...
    public function createImage(Request $request)
    {
        ...
        $image =  Upload::create([
            ...
            'user_id' => User::where('user_type', 'admin')
                ->first()->id, 'extension' => $image_type,
    ...
    public function login(Request $request)
    {
        if (Auth::attempt(['username' => request('username'), 'email' => function ($query) use ($request) {
            ...
            $image = Upload::where('user_id',$user->id)->first();
            if(isset($image))
            {
            ...
            $success['profileImage'] = $profileImage;
